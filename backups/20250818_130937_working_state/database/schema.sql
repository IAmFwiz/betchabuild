...
CREATE TABLE user_interactions (
  ...
  category TEXT NOT NULL,
  action TEXT CHECK (action IN ('yes', 'no', 'skip')),
  keywords TEXT[],
...
CREATE TABLE user_sessions (
  ...
  predictions_swiped INTEGER DEFAULT 0,
  predictions_placed INTEGER DEFAULT 0,
...
CREATE TABLE transactions (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID REFERENCES users(id),
  type TEXT CHECK (type IN ('deposit', 'withdrawal', 'fee', 'winning')),
...
-- Function to update preference scores
CREATE OR REPLACE FUNCTION update_preference_score()
RETURNS TRIGGER AS $$
BEGIN
  -- Calculate new preference score based on yes/no/skip counts
  NEW.preference_score = ((NEW.yes_count * 2) - NEW.no_count - (NEW.skip_count * 0.5)) / 
                         GREATEST(NEW.total_interactions, 1);
  NEW.updated_at = NOW();
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Trigger to automatically update preference scores
CREATE TRIGGER trigger_update_preference_score
  BEFORE UPDATE ON user_preferences
  FOR EACH ROW
  EXECUTE FUNCTION update_preference_score();

-- Function to start/end user sessions
...
BEGIN
  ...
  SET session_end = NOW() 
  WHERE user_id = user_uuid AND session_end IS NULL;
